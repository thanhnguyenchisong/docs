...
Là một kiểu tấn công web trong đó kẻ xấu lợi dụng việc browser tự động gửi cookie để giả mạo hành động của người dùng trên một site hợp lệ.

#### 🔑 Cách CSRF hoạt động
- Người dùng đã đăng nhập vào một site (ví dụ: bank.com) và có cookie session hợp lệ.
- Kẻ tấn công tạo một trang độc hại (evil.com) chứa form hoặc script gửi request đến bank.com.
- Khi người dùng truy cập evil.com, browser tự động gửi cookie của bank.com kèm theo request.
- Server bank.com nhận request, thấy cookie hợp lệ → tưởng là hành động thật của user.
- Kết quả: kẻ tấn công có thể thực hiện hành động thay người dùng (chuyển tiền, đổi mật khẩu…).
